...
namespace CalendarWebAPI.Localization
{
    /// <summary>
    ...
    internal class ResourceProviderXDocument : IResourceProvider<XDocument>
    {
        ...
        XDocument IResourceProvider<XDocument>.GetResource()
        {
            ...
            Stream resource = asm.GetManifestResourceStream("CalendarWebApi.Holiday.Localization.LocalizationString.xml");
            Result = XDocument.Load(resource);

#else

            string Texte;
                using (StreamReader reader = new StreamReader(new ResourceProviderXDocument().GetType().Assembly.GetManifestResourceStream("CalendarClassLibrary.Localization.LocalizationString.xml"), Encoding.UTF8))
                {
